...
class AbstractStriper {
  public:
    ...
    AbstractStriper() {}
    AbstractStriper(shared_ptr<StripeManager> s_m,
                    shared_ptr<ExtentManager> e_m)
        : stripe_manager(s_m), extent_manager(e_m), num_times_alternatives(0),
          num_times_default(0) {}

    virtual str_costs
    create_stripes(shared_ptr<AbstractExtentStack> extent_stack,
                   int simulation_time) {
        std::cerr << "virtual create_stripes should never happen";
        return str_costs();
    };
    virtual str_costs
    create_stripe(shared_ptr<AbstractExtentStack> extent_stack,
                  int simulation_time) {
        std::cerr << "virtual create_stripes should never happen";
        return str_costs();
    };
...
class SimpleStriper : public AbstractStriper {
  public:
    ...
    str_costs create_stripes(shared_ptr<AbstractExtentStack> extent_stack,
        ...
        int reads = 0;
        int stripes = 0;
        list<Extent *> exts_to_stripe =
            extent_stack->pop_stripe_num_exts(num_exts);
...
public:
    ...
    cost_to_replace_extents(int ext_size, vector<int> exts_per_locality,
        ...
        if (total_exts_replaced == stripe_manager->num_data_exts_per_stripe) {
            ...
            costs.local_parity_writes =
                stripe_manager->num_local_parities * ext_size;
            return costs;
        }
