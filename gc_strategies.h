...
struct stripe_gc_ret {
    short temp_space = 0, user_reads = 0, user_writes = 0,
          global_parity_reads = 0, global_parity_writes = 0,
          local_parity_reads = 0, local_parity_writes = 0,
          obsolete_data_reads = 0, absent_data_reads = 0,
          valid_obj_transfers = 0, storage_node_to_parity_calculator = 0,
          num_exts_replaced = 0;
    ...
    gc_handler_ret gc_handler(set<Stripe *> &stripe_set) override {
        ...
        for (auto stripe : stripe_lst) {
            ...
            if (obsolete >= primary_threshold && stripe != nullptr) {
                ...
                ret.total_valid_obj_transfers +=
                    stripe_gc_res.valid_obj_transfers;
                ret.total_storage_node_to_parity_calculator +=
                    stripe_gc_res.storage_node_to_parity_calculator;
                ret.total_global_parity_reads +=
                    stripe_gc_res.global_parity_reads;
                ret.total_global_parity_writes +=
                    stripe_gc_res.global_parity_writes;
                ret.total_local_parity_reads +=
                    stripe_gc_res.local_parity_reads;
                ret.total_local_parity_writes +=
                    stripe_gc_res.local_parity_writes;
                ret.total_obsolete_data_reads +=
                    stripe_gc_res.obsolete_data_reads;
        ...
        gc_handler_ret gc_handler(set<Stripe *> &stripe_set) override {
            ...
            for (auto stripe : stripe_lst) {
                ...
                if (obsolete >= primary_threshold && stripe != nullptr) {
                    ...
                    ret.total_obsolete_data_reads +=
                        stripe_gc_res.obsolete_data_reads;
                    ret.total_absent_data_reads +=
                        stripe_gc_res.absent_data_reads;
                    ret.total_num_exts_replaced +=
                        stripe_gc_res.num_exts_replaced;
                }
            }
            for (Stripe *d : deleted) {
                stripe_set.erase(d);
