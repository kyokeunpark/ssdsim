...
public:
  ...
  virtual str_costs get_stripe(float key = 0) {
    ...
    if (num_exts_at_key >= num_exts_per_stripe) {
      ...
      object_packer->generate_exts_at_key(extent_stack, num_exts_per_stripe,
                                          key);
      return striper->create_stripe(extent_stack, simulation_time);
    }
  }
  ...
  array<double, 2> get_proporition_of_stripers() {
    int num_times_alternative =
        striper->num_times_alternatives + gc_striper->num_times_alternatives;
