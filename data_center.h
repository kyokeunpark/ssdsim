  ...
  public:
    ...
    eh_result event_handler() {
        ...
        while (configtime <= (int)this->simul_time &&
            ...
            for (auto it : this->obs_by_ext_types) {
                if (net_obs_by_ext_type.find(it.first) !=
            ...
            obs_perc = -1;
            if (used_space > 0)
                obs_perc = (added_obsolete_this_gc + net_obsolete) /
                           (double)used_space * 100;
        ...
        cout << "Number of extents in dc: " << this->ext_mngr->get_num_ext()
             << endl;
        cout << "Number of stripes in dc: "
             << this->stripe_mngr->get_num_stripes() << endl;
    ...
    sim_metric run_simulation() {
        ...
        ret.total_parity_calculator_to_storage_node =
            eh.total_valid_obj_transfers;
        ...
        ret.total_user_data_writes =
            ret.total_user_data_reads * coding_overhead;
        ...
        if (ret.total_reclaimed_space > 0)
            ret.gc_amplification =
                ret.total_gc_bandwidth / ret.total_reclaimed_space;
        else
        ...
        for (auto it : ret.types) {
            string key = it.first;
            int val = it.second;
            if (valid_objs_by_ext_type.find(key) !=
                valid_objs_by_ext_type.end())
        ...
        for (auto &it : this->obs_by_ext_types) {
            ...
            cout << "Obs \% per ext for " << it.first << ": " << it.second
                 << endl;
